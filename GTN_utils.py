...
# define GTN-based encoder and decoder
#define model
def GTN_encoder_decoder_classifier(A, F, params, num_labels):
    num_nodes = int(F.shape[1])
    model_layers = params[0]
    pooling_type = params[1][0]
    concatenate = params[1][1]
    input_dim, output_dim = model_layers[0], model_layers[-1]
    num_layers = len(model_layers) - 1
    
    layer_output = []
    
    for layer in range(1, len(model_layers)):
        input_dim = model_layers[layer-1]
        output_dim = model_layers[layer]
